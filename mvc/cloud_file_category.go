...
func CloudFileCategory(db *Sql, value string) (data []File, e error) {
	...
	// 查询
	rows, err := db.DB.Query("select id,IFNULL(user_id,'null'),IFNULL(file_name,'null'),IFNULL(parent_id,0),IFNULL(ptime,0),IFNULL(file_cid,'null'),IFNULL(file_size,0),IFNULL(file_type,0),IFNULL(is_folder,0),IFNULL(thumbnail,'null'),IFNULL(width,''),IFNULL(height,''),IFNULL(duration,0) from cloud_file where file_type=? and user_id=? order by ?", list.FileType, userid, or)
	if err != nil {
